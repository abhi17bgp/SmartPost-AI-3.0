  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-background border-l border-border">
        ...
        <p className="mt-4 text-muted-foreground text-sm font-medium animate-pulse tracking-widest uppercase">Sending Request</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-background border-l border-border">
        ...
        <h3 className="text-xl font-medium text-muted-foreground mb-2 tracking-tight">Ready to Send</h3>
        <p className="text-muted-foreground/60 text-sm max-w-xs text-center">Hit the Send button to execute the request and see the response here.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-background border-l border-border">
      <div className="px-4 py-3 flex items-center justify-between border-b border-border bg-background/90 backdrop-blur shrink-0 sticky top-0 z-10">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider flex items-center gap-1"><Clock size={12} /> Time</span>
            <span className="font-mono text-sm text-primary font-bold">{data.timeTaken}<span className="text-primary/60 font-medium text-xs ml-0.5">ms</span></span>
          </div>
          <div className="w-[1px] h-5 bg-border hidden sm:block" />
          <div className="hidden sm:flex items-center gap-2">
            <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider flex items-center gap-1"><Server size={12} /> Size</span>
          ...
          <div className="p-4 grid grid-cols-[1fr_2fr] gap-x-4 gap-y-2 font-mono text-xs max-w-3xl">
            {Object.entries(data.headers || {}).map(([key, value]) => (
              <React.Fragment key={key}>
                <div className="text-muted-foreground border-b border-border/50 py-1.5 break-all">{key}</div>
        ...
        {activeTab === 'AI Analysis' && (
          <div className="p-6 h-full overflow-y-auto custom-scrollbar bg-background">
            {aiLoading ? (
              ...
              <div className="prose dark:prose-invert prose-emerald max-w-none text-sm leading-relaxed prose-pre:bg-muted/50 prose-pre:border prose-pre:border-border prose-a:text-purple-500 prose-headings:text-foreground prose-strong:text-purple-600 dark:prose-strong:text-purple-400 prose-code:text-primary animate-fade-in break-words">
